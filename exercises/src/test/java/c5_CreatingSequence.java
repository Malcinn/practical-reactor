...
public class c5_CreatingSequence {
    ...
    public void interval() {
        Long[] longs = {0L,1L,2L};
        Flux<Long> interval = Flux.fromArray(longs)
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Flux.generate(synchronousSink -> {
            try {
                synchronousSink.next(callable.call());
            } catch (Exception e) {
    ...
    public void generate_programmatically() {
        ...
        AtomicInteger counterGenerate = new AtomicInteger(0);
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            sink.next(counterGenerate.getAndIncrement());
            if (counterGenerate.get() == 6){
                sink.complete();
            }
        });
